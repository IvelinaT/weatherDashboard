...
describe('User::Controller', () => {
  ...
  context('getUserById', () => {
    ...
    it('should log and throw an NotFoundError when the query returns no results for the id passed in', async () => {
      try {
        await user.getUserById({ id: 2, excludeDeleted: true })
      } catch (error) {
        expect(error).to.be.instanceOf(NotFoundError).with.property('message', 'Requested User Not Found')
      }
    })
    it('should log and throw an error when the query fails', async () => {
      try {
